...
class Usdt extends _i1.GeneratedContract {
  ...
  Future<BigInt> CHILD_CHAIN_ID({_i1.BlockNum? atBlock}) async {
    final function = self.abi.functions[1];
    assert(checkSignature(function, '626381a0'));
    final params = [];
  ...
  /// set, the function will be evaluated in the specified block. By default, the
  /// latest on-chain block will be used.
  Future<_i2.Uint8List> CHILD_CHAIN_ID_BYTES({_i1.BlockNum? atBlock}) async {
    final function = self.abi.functions[2];
    assert(checkSignature(function, '0b54817c'));
    final params = [];
    final response = await read(
      function,
      params,
  ...
  /// set, the function will be evaluated in the specified block. By default, the
  /// latest on-chain block will be used.
  Future<_i2.Uint8List> DEPOSITOR_ROLE({_i1.BlockNum? atBlock}) async {
    final function = self.abi.functions[4];
    assert(checkSignature(function, 'a3b0b5a3'));
    final params = [];
    final response = await read(
      function,
      params,
  ...
  /// set, the function will be evaluated in the specified block. By default, the
  /// latest on-chain block will be used.
  Future<BigInt> ROOT_CHAIN_ID({_i1.BlockNum? atBlock}) async {
    final function = self.abi.functions[6];
    assert(checkSignature(function, '8acfcaf7'));
    final params = [];
    final response = await read(
      function,
      params,
  ...
  /// set, the function will be evaluated in the specified block. By default, the
  /// latest on-chain block will be used.
  Future<_i2.Uint8List> ROOT_CHAIN_ID_BYTES({_i1.BlockNum? atBlock}) async {
    final function = self.abi.functions[7];
    assert(checkSignature(function, '0dd7531a'));
    final params = [];
    final response = await read(
      function,
      params,
  ...
  Future<String> changeName(
    ...
    assert(checkSignature(function, '5353a2d8'));
    final params = [name_];
    return write(
      credentials,
      transaction,
  ...
  Future<String> deposit(
    ...
    _i2.Uint8List depositData, {
    required _i1.Credentials credentials,
    _i1.Transaction? transaction,
  }) async {
    final function = self.abi.functions[14];
    assert(checkSignature(function, 'cf2c52cb'));
    final params = [
      user,
      depositData,
    ];
  ...
  Future<String> initialize(
    ...
    String symbol_,
    BigInt decimals_,
    _i1.EthereumAddress childChainManager, {
    required _i1.Credentials credentials,
    _i1.Transaction? transaction,
  }) async {
    final function = self.abi.functions[25];
    assert(checkSignature(function, 'de7ea79d'));
    final params = [
      name_,
      symbol_,
      decimals_,
